...
namespace LIst_task
{
    class List
    {
        ...
        public void Prepend(int elementValue)
        {
            ...
            if (count == 0)//проверяем не пустой ли список
            {
                Node node = new Node(null, null, elementValue);//создаем новый узел(он сразу первый и последний)
                last = node;
                first = last;
                count++;
        ...
        public void Append(int elementValue)
        {
            ...
            else
            {
                ...
                last.Next = node;//обращаемся к параметру переменной
                last = node;
                count++;
            }
        }
        ...
        {
            int i = 0;
            Node current = first;//текущий элемент который смотрим
            Node prev = first.Prev;// предыдущий
            while (current != null)
            {
